...
- **Фаза:** 2 (фронтенд + вертикальный срез) завершена — каталог/карточка/главная/
  корзина/оформление/автовыдача `stock` готовы. Дальше — Фаза 3 (режимы api/manual,
  админка, отзывы, поиск).
- **Авторизация:**
