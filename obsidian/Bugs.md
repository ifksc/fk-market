...
- 2026-05-16 — Telegram-логин падал с «Подпись id_token не сошлась» когда JWKS-кэш (TTL 1 час) истекал в момент сетевого флака маршрута до oauth.telegram.org. Фикс: `fetchOauthJwks` — двухуровневый кэш, при сбое сети отдаём stale-копию (TTL 30 дней) вместо исключения. retry усилен 2→3 на всех oauth-вызовах.
...
- 2026-05-16 — На странице товара дублировался текст описания. Корень: у товаров из синхронизации fkwallet `short_description` — это обрезанная копия начала `description` (обрывается на полуслове). Вкладка «Описание» в `ProductTabs` рендерила оба поля, а защита `description !== shortDescription` ловила только полное равенство, не префикс. Фикс: вкладка показывает только `description` (полный текст), `short_description` — лишь запасной вариант, если полного описания нет.
- 2026-05-17 — `app/loading.tsx` сломал ВСЕ редиректы и 404 на сайте. `loading.tsx` (добавлен в R3.1) включает стриминг на каждом роуте — HTTP 200 + loading-скелет flush'атся клиенту до завершения рендера, и серверные `permanentRedirect`/`notFound` уже не могут изменить статус. Итог: `/catalog?category=X`, legacy-slug товаров (200+ переименованных) и категорий отдавали 200 вместо 308; несуществующие страницы — 200 вместо 404. Найдено бинарным поиском (убирал error.tsx/loading.tsx по одному; мешал EADDRINUSE — старый `next start` не убивался). Фикс: удалить `loading.tsx` (PR #90). Доп.: фронт-`rsync` был без `--delete` → удалённый файл остался на сервере, `loading.tsx` убран вручную + добавлен `--delete` (PR #91).
- 2026-05-17 — Медленные api-выдачи (Telegram Звёзды) ложно падали в ручную очередь с «⚠ API упал: Polling timeout», хотя cron их штатно дозабивал. Корень: `FulfillViaApiJob::pollUntilDone` при таймауте бросал исключение, а `catch (\Throwable)` в `handle()` ловил его наравне с реальными сбоями → `fallbackToManual`. После фазы R2 (polling сокращён до ~60с) почти каждая медленная выдача упиралась в таймаут. Фикс: таймаут polling — не ошибка, `pollUntilDone` просто завершается, позиция остаётся `in_progress` для крона; реальные сбои по-прежнему бросают исключение.
- 2026-05-16 — Гостевая форма поддержки показывала `API /support → 422` вместо понятного текста. Корень: `apiFetch` в `frontend/lib/api.ts` бросал generic-ошибку `API ${path} → ${status}`, не читая поле `message` из тела ответа Laravel. Фикс: при `!res.ok` достаём `message` из JSON-тела и показываем его (фолбэк на технический текст, если тело не JSON). Бэкенд при этом работал верно — отдавал 422 «Заказ с таким номером и email не найден».
