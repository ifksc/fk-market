...
import { AuthError, oauthYandexExchange } from '@/lib/auth';
import { popYandexOAuthSession } from '@/lib/yandex-oauth';
...
function Inner() {
  ...
  useEffect(() => {
    ...
    const errorFromYandex = params.get('error') ?? params.get('error_description');

    if (errorFromYandex) {
      setError(`Яндекс отказал: ${errorFromYandex}`);
      return;
    }
    if (!code || !state) {
      setError('Яндекс не вернул код входа');
      return;
    }
    ...
    (async () => {
      try {
        const { user, needs_email } = await oauthYandexExchange({
          code,
      ...
      <div className="min-h-[60vh] flex items-center justify-center p-4">
        <div className="w-full max-w-sm text-center">
          ...
          <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-2xl p-8 shadow-xl">
            <h1 className="text-lg font-semibold mb-3">Войти через Яндекс не получилось</h1>
  ...
  return (
    <div className="min-h-[60vh] flex items-center justify-center">
      <div className="text-center">
        ...
        <p className="text-sm text-gray-500">Завершаем вход через Яндекс…</p>
      </div>
    </div>
  );
}
